...
const restaurants = [
...

    {
        ...
        budget:1100,
        distance:5,
        image:"https://images.unsplash.com/photo-1604908177522-040d8e8f5c0b",
...
function generateRestaurant(){
    ...
    const filteredRestaurants = restaurants.filter(r => {
        ...
        let typeMatch = r.type === foodType;

        const budgetMatch = r.budget <= budget;
        const distanceMatch = r.distance <= distance;
        const dessertMatch = dessert === "No" ? true : r.dessert;

        return typeMatch && budgetMatch && distanceMatch && dessertMatch;
    });
        ...
        <div class="restaurant-card">
            ...
            <div class="restaurant-content">
                ...
                <div class="restaurant-buttons">
                    <button onclick="openMenu('${r.menu}')">View Menu</button>
                    <button onclick="openMaps('${r.maps}')">Open GPS Route</button>
                    <button onclick="saveRestaurant('${r.name}')">Save Favorite</button>
                </div>
            </div>
        </div>`;
    });
